		...
		frappe.call({method:"seabridge_app.seabridge_app.api.get_invoice_details",
			...
			callback:function(r){
				var mockup_window=new frappe.ui.Dialog({title:__('<a href="desk#Form/Purchase%20Invoice/'+invoice+'" style="text-color:blue;" target="_blank"><div style="color:blue;">'+invoice+'</div></a>'),
					fields:[{fieldname:"company",label:__("Company"),fieldtype:"Data",read_only:1},
						{fieldname:"description",label:__("Invoice Description"),fieldtype:"Small Text",read_only:1},
						{fieldname:"po_document",label:__("PO Document"),fieldtype:"HTML",read_only:1},
						{fieldname:"po_amount",label:__("PO$"),fieldtype:"Data",read_only:1},
						{fieldname:"due_date",label:__("Invoice Date"),fieldtype:"Data",read_only:1},

						{fieldname:"column_break",fieldtype:"Column Break"},
						{fieldname:"supplier",label:__("Vendor"),fieldtype:"Data",read_only:1},
						{fieldname:"po_date",label:__("PO Date"),fieldtype:"Data",read_only:1},
						{fieldname:"invoice_amount",label:__("Inv$"),fieldtype:"Data",read_only:1},
						{fieldname:"budget",label:__("Budget"),fieldtype:"Data",read_only:1},
				...
				for(var attachment=0; attachment<r.message[1].length; attachment++){
					attachments+='<a href="'+r.message[1][attachment][1]+'" Download><font style="color:blue;">'+r.message[1][attachment][0]+'</font></a><br>'
				}
				var po_doc='<font style="font-size: 12px;color:#9E9E9E;">PO Document</font><br><label style="background-color: #EDF2F2; color: blue;width: 260px;height: 29px;"><div style="color: blue;padding-top: 5px;padding-left: 9px;"></div></label>'
				if(r.message[0][0][4]){
				po_doc='<font style="font-size: 12px;color:#9E9E9E;">PO Document</font><br><label style="background-color: #EDF2F2; color: blue;width: 260px;height: 29px;"><a href="desk#Form/Purchase%20Order/'+r.message[0][0][4]+'" target="_blank"><div style="color: blue;padding-top: 5px;padding-left: 9px;">'+r.message[0][0][4]+'</div></a></label>'
				}
				mockup_window.get_field("company").set_value(r.message[0][0][0])
				mockup_window.get_field("po_document").set_value(po_doc)
				mockup_window.get_field("po_amount").set_value(r.message[0][0][5])
				mockup_window.get_field("due_date").set_value(r.message[0][0][2])
				mockup_window.get_field("supplier").set_value(r.message[0][0][1])
				mockup_window.get_field("po_date").set_value(r.message[0][0][6])
				mockup_window.get_field("invoice_amount").set_value(r.message[0][0][3])	
				mockup_window.get_field("budget").set_value(r.message[0][0][7])	
				mockup_window.get_field("description").set_value(r.message[0][0][8])
				if(r.message[1]!=0){
