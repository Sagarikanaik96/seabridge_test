!function(){"use strict";
frappe.templates.item_dashboard='<div> <div class="dashboard-list-item">   <div class="row"><div class="col-sm-1 small" style="margin-top: 8px;"><center><input type="checkbox" id="chcb1" name="chcb1" class="chcb1"></center></div><div class="col-sm-2 small" style="margin-top: 8px;margin-left:-20px;"><b>Vendor</b></div><div class="col-sm-2 small" style="margin-top: 8px;margin-left:-20px;"><b>Inv#</b></div>   <div class="col-sm-1 small" style="margin-top: 8px;text-align:right;"><b>Inv$</b></div><div class="col-sm-1 small" style="margin-top: 8px;text-align:right;"><b> Budget </b></div><div class="col-sm-1 small" style="margin-top: 8px;">  <b>PO Date</b>  </div><div class="col-sm-1 small" style="margin-top: 8px;"> <b> Invoice Due Date </b> </div><div class="col-sm-1 small" style="margin-top: 8px;"> <a data-type="match"> <b> Match ? </b></a></div><div class="col-sm-1 small" style="margin-top: 8px;">  <b>Pending With  </b></div><div class="col-sm-1 small" style="margin-top: 8px;"><b>Status</b></div>   </div></div> <div class="result"></div>  <div id="btn-more" class="more"  style="padding: 15px;"> <a class="btn btn-default btn-xs btn-more">More</a>  </div> </div>  <center><div id="records" class="records" style="font-size:90%;color:grey;"></div> <center><table><tr><td><div class="submit"><button id="btn-pay" class="btn btn-default btn-xs btn-pay  button-sbt button"  style="display:none;">Create Payment</a>    </div></td></tr></center>',frappe.templates.item_dashboard_list='<style>input{border: none;background: transparent;}.button-sbt {background-color: #008CBA;}.button-apr {background-color: #4CAF50;}.button {border: none; color: white; padding: 6px 6px;text-align: center;font-size: 10px;margin: 2px 2px;}div.b {word-wrap: break-word;}.hover{cursor: pointer;margin: 15px 0;}</style>{% for d in data %}  <div class="dashboard-list-item">   <div class="row"> <div class="col-sm-1 small" style="margin-top: 8px;"><center>{%if d[14]==1 %}<input type="checkbox" id="chcb2" name="chcb2" class="btn-check" disabled>{%else%}<input type="checkbox" id="chcb2" name="chcb2" class="btn-check">{%endif%}</center></div>  <div class="col-sm-2 small" style="margin-top: 8px;margin-left:-20px;word-wrap: break-word;">     <a data-type="supplier" data-name="{{ d.supplier }}">{{ d[1] }}</a>   </div>     <div class="col-sm-2 small" style="margin-top: 8px;margin-left:-20px;">  <div style="color:blue;">        <label value="{{d[0]}}" style="color:blue;" id="inv" class="inv"><font style="color:blue;" class="hover">{{d[0]}}</font></label>&nbsp;{%if (d[12]>0)%}<i class="fa fa-paperclip" aria-hidden="true"></i>{%endif%}</div></div>  <div class="col-sm-1 small" style="margin-top: 8px;text-align:right;word-wrap: break-word;"> {{ d[2]}} </div> <div class="col-sm-1 small" style="margin-top: 8px;text-align:right;">  {{ d[9] }}  </div><div class="col-sm-1 small" style="margin-top: 8px;word-wrap: break-word;">  {{d[6]}}  </div> <div class="col-sm-1 small" style="margin-top: 8px;word-wrap: break-word;">  {{ d[3] }}  </div><div class="col-sm-1 small" style="margin-top: 8px;">  <a data-type="match">{%if d[7]==1%}Y{%else%}N{%endif%} </a> </div> <div class="col-sm-1 small" style="margin-top: 8px;word-wrap: break-word;">  {{ d[8] }}  </div><div id="btn-status" class="col-sm-1" style="margin-top: 8px;text-align:left;word-wrap: break-word;" name="{{d[1]}}" data-type="{{d[1]}}"> </a>         {%if (d[4]=="To Pay")%} <button  class="btn btn-default btn-xs btn-add btn-success btn-status" value="{{d[1]}}" style="background-color: #4CAF50;">{{d[4]}} </button>{%else%}{%if (d[4]=="Rejected")%} <button  class="btn btn-default btn-xs btn-add btn-danger" style="background-color:#FF0000;" onclick="try_function()">{{d[4]}} </button>{%else%}{%if (d[4]=="Draft")%} <button  class="btn btn-default btn-xs btn-info btn-status" onclick="try_function()" style="background-color:#8D99A6;">{{d[4]}} </button>{%else%}<button  class="btn btn-default btn-xs btn-add btn-warning" style="background-color:#FFA00A;"  onclick="try_function()">{{d[4]}}</button> {%endif%} {%endif%} {%endif%}</div> </div><div class="row"><div class="col-sm-3 small"></div><div class="col-sm-9 small" style="margin-left:-40px;">{{d[13]}} </div></div> </div> {% endfor %}',
...
function t(t,a){
	...
	this.content.on("click",".btn-pay",function(){
		...
		frappe.call({method:"seabridge_app.seabridge_app.api.create_payment",
					...
					if(r.message!=0){
						...
						window.location.href = "/desk#Form/Bank%20Payment%20Advice/"+r.message
					}
			    	    }
				})
	...
	refresh:function(){
	    ...
	    var e=this;
	if(this.company!="" && this.mode_of_payment!=""){
