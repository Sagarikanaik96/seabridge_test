...
function t(t,a){
	...
	this.content.on("click",".inv",function(){
		...
		frappe.call({method:"seabridge_app.seabridge_app.api.get_invoice_details",
			...
			callback:function(r){
				var mockup_window=new frappe.ui.Dialog({title:__(invoice),
					fields:[{fieldname:"company",label:__("Company"),fieldtype:"Data",read_only:1},
						{fieldname:"po_document",label:__("PO Document"),fieldtype:"Link",options:"Purchase Order",read_only:1},
						{fieldname:"po_amount",label:__("PO Amount"),fieldtype:"Data",read_only:1},
						{fieldname:"due_date",label:__("Due Date"),fieldtype:"Data",read_only:1},
						...
						{fieldname:"invoice_amount",label:__("Invoice Amount"),fieldtype:"Data",read_only:1},
				...
				for(var attachment=0; attachment<r.message[1].length; attachment++){
					attachments+='<a href="'+r.message[1][attachment][1]+'" Download>'+r.message[1][attachment][0]+'</a><br>'
				}
				mockup_window.get_field("company").set_value(r.message[0][0][0])
				mockup_window.get_field("po_document").set_value(r.message[0][0][4])
				...
				if(r.message[1]!=0){
					mockup_window.get_field("attachments").set_value('<b>Attachments</b><br>'+attachments)
				}
	   		}
		})
