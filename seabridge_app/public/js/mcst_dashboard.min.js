!function(){"use strict";
frappe.templates.mcst_dashboard='<div> <div class="dashboard-list-item">   <div class="row"><div class="col-sm-1 small" style="margin-top: 8px;"><center><input type="checkbox" id="chcb1" name="chcb1" class="chcb1"></center></div><div class="col-sm-3 small" style="margin-top: 8px;margin-left:-20px;"><b>BPA Number</b></div><div class="col-sm-2 small" style="margin-top: 8px;"><b>Date</b></div> <div class="col-sm-2 small" style="margin-top: 8px;text-align:right;"><b>Total Approvals Required</b></div><div class="col-sm-2 small" style="margin-top: 8px;text-align:right;"><b>Current Approves</b></div><div class="col-sm-2 small" style="margin-top: 8px;text-align:left;margin-left:20px;"><b> Status </b></div> </div></div><div class="result"></div>  <div id="btn-more" class="more"  style="padding: 15px;"> <a class="btn btn-default btn-xs btn-more">More</a>  </div> </div>  <center><div id="records" class="records" style="font-size:90%;color:grey;"></div> <center><table><tr><td><div class="submit"><button id="btn-approve" class="btn btn-default btn-xs btn-pay  btn-approve button"  style="display:none;background-color:#4CAF50;width: 160px;font-size: 12px;">Approve</a>    </div></td></tr></center>',frappe.templates.item_dashboard_list='<style>input{border: none;background: transparent;}.button-sbt {background-color: #008CBA;}.button-apr {background-color: #4CAF50;}.button {border: none; color: white; padding: 6px 6px;text-align: center;font-size: 10px;margin: 2px 2px;}div.b {word-wrap: break-word;}.hover{cursor: pointer;margin: 15px 0;}</style>{% for d in data %}  <div class="dashboard-list-item">   <div class="row"> <div class="col-sm-1 small" style="margin-top: 8px;"><center><input type="checkbox" id="chcb2" name="chcb2" class="btn-check"></center></div>  <div class="col-sm-3 small" style="margin-top: 8px;margin-left:-20px;word-wrap: break-word;" id="bpa">     <a data-type="bpa" data-name="{{ d[0] }}" href="/desk#Form/Bank%20Payment%20Advice/{{d[0]}}" target="_blank"><font style="color:blue;" class="hover"><b>{{d[0]}}</b></font></a>   </div>   <div class="col-sm-2 small" style="margin-top: 8px;word-wrap: break-word;"> {{ d[1]}} </div>  <div class="col-sm-2 small" style="margin-top: 8px;text-align:right;word-wrap: break-word;"> {{ d[3]}} </div><div class="col-sm-2 small" style="margin-top: 8px;text-align:right;word-wrap: break-word;"> {{ d[6]}} </div><div class="col-sm-2 small" style="margin-top: 8px;word-wrap: break-word;margin-left:20px;"> {%if (d[4]=="Draft")%} <button id="btn-status" class="btn btn-default btn-xs btn-info btn-status" value="{{d[1]}}" style="background-color:#8D99A6;">{{d[4]}}</button>{%else%}<button id="btn-status" class="btn btn-default btn-xs btn-add btn-warning btn-status" style="white-space: normal;" value="{{d[1]}}" style="background-color:#FFA00A;">{{d[4]}}</button> {%endif%}</div></div></div>{% endfor %}',
	...
	this.content.on("click",".btn-approve",function(){
		...
		for(var i=0; i< allBPA.length; i++){
			if(allCB[i].checked==true){
				console.log(allBPA[i].textContent.trim())
		...
		if(bpas.length>0){alert('You dont have permission to approve the documents  '+bpas+' as you have already approved the documents once.');}
