	...
	setup: (frm) => {
		frm.make_methods = {
			'Landed Cost Voucher': () => {
				...
				lcv_receipt.receipt_document_type = 'Service Completion Note';
                    ...
                    $.each(tabletransfer.items, function(index, row){
                        if(item_code==row.item_code){
                            ...
                            frappe.validated = false;
                             msgprint('This document is over limit by <b>Qty '+check+'</b> for item <b>'+item.item_code+'</b>. Are you making another <b>Service Completion Note</b> against the same <b>Purchase Order Item</b>? <br><br> To allow over receipt, update "Over Receipt Allowance" at Service Completion Note Item details','Limit Crossed')
                            }
                            }
                        }
                    })
		...
		frappe.call({
			...
			callback: function (r) {
				if (r.message) {
					...
					frappe.msgprint(__("Service Completion Note doesn't have any Item for which Retain Sample is enabled."));
				}
			}
		});
	},
...
cur_frm.cscript.select_print_heading = function(doc, cdt, cdn) {
	if(doc.select_print_heading)
		...
		cur_frm.pformat.print_heading = "Service Completion Note";
}
...
frappe.ui.form.on("Service Completion Note", "is_subcontracted", function(frm) {
